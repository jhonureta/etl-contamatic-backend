...
CASE 
WHEN SUBSTRING_INDEX(detalle_anticipos.FK_ORDEN, '-', 1) IN('OE', 'OV') THEN 'ANT-ORDEN' ELSE 'ANTPROV'
END AS ORIGEN_MOVI,
ABS(
...
END AS ESTADO_MOVI,
CASE WHEN PER_BENE_MOVI = '' THEN proveedores.NOM_PROV ELSE PER_BENE_MOVI
END AS PER_BENE_MOVI,
CAUSA_MOVI,
#CASE WHEN ORIGEN_MOVI = 'ANTICIPOS-CLIENTES' THEN 'ANTPROV' ELSE ORIGEN_MOVI
case
WHEN SUBSTRING_INDEX(detalle_anticipos.FK_ORDEN, '-', 1) IN('OE', 'OV') THEN 'ANT-ORDEN' ELSE 'ANTPROV'
END AS MODULO,
CASE
WHEN SUBSTRING_INDEX(detalle_anticipos.FK_ORDEN, '-', 1) IN('OE') THEN 'ANT-ORDEN-TRABAJO' 
WHEN SUBSTRING_INDEX(detalle_anticipos.FK_ORDEN, '-', 1) IN('OV') THEN 'ANT-ORDEN-VENTA' ELSE 'ANTPROV'
END AS ORG_ORDEN,
...
INNER JOIN proveedores ON proveedores.ID_PROV = anticipos.FK_COD_PROV_ANT
WHERE
    anticipos.TIPO_ANT = 'PROVEEDORES' AND ORIGEN_MOVI <> 'CxP';`);
        ...
        const movementSequenceQuery = await conn.query(`SELECT MAX(SECU_MOVI)+1 AS SECU_MOVI FROM movements WHERE MODULO = 'ANTPROV' AND  FK_COD_EMP = ?`,
            [newCompanyId]
        );
...
WHEN SUBSTRING_INDEX(da.FK_ORDEN, '-', 1) IN('OV') THEN 'ANT-ORDEN-VENTA' ELSE 'ANTPROV'
END AS ORG_ORDEN,
...
INNER JOIN proveedores p 
    ON p.ID_PROV = a.FK_COD_PROV_ANT
...
export async function migrateAccountingEntriesCustomerObligations(
    ...
    CASE WHEN SUBSTRING_INDEX(
        detalle_anticipos.FK_ORDEN,
        '-',
        1
    ) IN('OE', 'OV') THEN 'ANT-ORDEN' ELSE 'ANTPROV'
END AS ORG_ASI,
...
INNER JOIN proveedores ON proveedores.ID_PROV = anticipos.FK_COD_PROV_ANT
WHERE
    origen_asiento = 'ANTTICIPOS PROV';` );
...
export async function migrateDetailedAccountingEntriesCustomerObligations(
    ...
    console.log("🚀 Iniciando migración de detalles de asientos contables anticipo proveedores..........");
...
WHERE
    ...
    for (let i = 0; i < rows.length; i += BATCH_SIZE) {
        ...
        try {
            ...
            for (const o of batch) {
                const idPlan = mapAccounts[o.FK_CTAC_PLAN];
                console.log(`➡️ Procesando detalle de asiento anticipo proveedores ${idPlan}`);
            ...
            console.log(`✅ Batch ${i / BATCH_SIZE + 1} procesado anticipo proveedores`);
    ...
    console.log("🎉 Migración  detalles contables completada anticipo proveedores");
    return { mapAccountDetail };
}