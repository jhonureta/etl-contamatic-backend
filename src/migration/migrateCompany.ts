  ...
  try {
    const [rowsEmpresaLegacy] = await legacyConn.query(
      `SELECT *, 1 AS FK_COD_COUNT,'ACTIVO' AS EST_EMP, 'CORPNEWBEST' as CONF_FIRM_PROV, NOMFIRELEC_EMP as CONF_FIRM, CLAVFIRELEC_EMP as CONF_FIRM_PASS,  FECEMISION_FIR AS CONF_FIRM_EMI, FECVENCE_FIR AS CONF_FIRM_EXP,TIPAMB_EMP AS CONF_TIPAMB,
            ...
            LEGEND_EMP as CONF_CONT_LEYEND,'#000000' as COLOR_LEYENDA, '#1b6898' as CONF_COLOR_LETR, DATOS_EXTR_EMP ,'[]' as CONF_ITEMS_ADIC, 0 AS CONF_TICKET_AUTOM, 2 AS CONF_BARCODE_NUM, 0 AS CONF_BARCODE_COLOR,  CIERRE_CAJA as CLOSING_BOX, 1 as CONF_TYPE_POINTSALES,(NOW()) AS FECHA_CREACION FROM empresa limit 1`,
    );
